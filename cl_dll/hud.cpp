...
public:
	virtual void GetPlayerTextColor(int entindex, int color[3])
	{
		...
		sscanf(CVAR_GET_STRING("cl_hudcolor"), "%i %i %i %i", &hud_color[0], &hud_color[1], &hud_color[2], &hud_color[3] );
...
const char* CHud::RemoveColors( char* pBuf ) {
	...
	for( int i = 0; i < pText.length(); i++ ) {
		if( pText.substr(i, 1) == "^" ) {
			if( pText.substr(i, 2) != "^0" && !atoi(pText.substr(i+1, 1).c_str()) ) {
				continue;
			}
			pText.erase(i, 2);
		}
